...
public class PaymentHome {
	...
	public Payment merge(Payment detachedInstance) {
		...
		try {
			Payment result = (Payment) getSessionFactory().getCurrentSession().merge(detachedInstance);
	...
	public Payment findById(java.lang.Integer id) {
		...
		try {
			Payment instance = (Payment) getSessionFactory().getCurrentSession().get("Payment", id);
			if (instance == null) {
	...
	public List findByExample(Payment instance) {
		...
		try {
			List results = getSessionFactory().getCurrentSession().createCriteria("Payment").add(Example.create(instance))
					.list();

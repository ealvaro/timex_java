...
public class ApproveTimesheetsController extends SimpleFormController {
	...
	protected Object formBackingObject(HttpServletRequest request) {
		...
		List<Employee> reportingEmployees = employeeManager.getReportingEmployees(employee.getEmployeeId());
		...
		for (Employee e : reportingEmployees) {
			...
			for (Timesheet t : empTimesheets) {
				if (t.getPeriodEndingDate().equals(DateUtil.getCurrentPeriodStartingDate())
						|| t.getStatusCode().equalsIgnoreCase(Timesheet.SUBMITTED)
						|| t.getStatusCode().equalsIgnoreCase(Timesheet.DISAPPROVED)
						|| t.getStatusCode().equalsIgnoreCase(Timesheet.APPROVED))
					timesheets.add(t);
			}
		}
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command,
		...
		for (Timesheet t : timesheets) {
			...
			if (t.getStatusCode().equalsIgnoreCase(Timesheet.APPROVED)) {
